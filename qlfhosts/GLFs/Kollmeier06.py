...
class Kollmeier06(AGNLeddBased):
    ...
    def P(self, Lh_nu_max, **kargs):
        ...
        #Get the minimum l_lambda_min we need to be able to detect the AGN. This is based on estimating the maximum BH mass possible based on the maximum host luminosity allowed.
        l_lambda_min = self.l_lambda_min(Lh_nu_max, **kargs)
        if l_lambda_min is None:
            return [None]*len(Lh_nu_max)
