...
class SelectionCriteria(object):
    ...
    def is_selected(self,Lhost_Lagn, Alam, lam_rest, agn_sed, hosts_sed, k_hosts_sed):
        ...
        for j, bp in enumerate(agn_sed.bps):
            ...
            Aband[j] = agn_sed.klam(lam_eff_band/(1+agn_sed.z)) / klam_norm

        #Get the AGN magnitudes.
        agn_mags = dict()
        for j, bp_name in agn_sed.bp_names:
            agn_mags[bp_name] = agn_sed.mag_unscaled[0][j] + Aband[j]

        #Get the host magnitudes. 
        host_mags = dict()
        for j, bp_name in hosts_sed.bp_names:
        ...
        try:
            ...
            for bp_name in agn_mags:
                mag[bp_name] = -2.5*np.log10(10**(-0.4*agn_mags[bp_name]) + Lhost_Lagn * 10**(-0.4*host_mags[bp_name]))
        except KeyError:
